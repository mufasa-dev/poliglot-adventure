<script>
  ...
  $: currentItem = exercise.content[currentIndex];
...
<div class="max-w-2xl mx-auto p-4 bg-bg-secondary rounded-lg shadow">
  <div class="flex items-center gap-2 mb-2">
    <ProgressBar current={currentIndex} total={exercise.content.length} showLabel={false} />
  ...
  <h1 class="text-2xl font-bold mb-2 mt-4">{exercise.title}</h1>
  ...
  {#if !showResult}
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      {#if currentItem.type === 'message'}
        ...
        {/if}
      {:else}
        <p class="mb-2">{currentItem.question}</p>
  ...
  {:else}
    ...
    <div class="bg-white p-6 rounded-lg shadow text-center">
      <h2 class="text-2xl font-bold mb-4">Parabéns! Você concluiu o exercício 🎉</h2>
      <p class="mb-2">Acertou {correctAnswers} de {totalQuestions} questões.</p>
      <p class="text-xl font-semibold">Pontuação: {scorePercent}%</p>
      <button on:click={backScreen} class="btn-primary">
          <FontAwesomeIcon icon={faCaretLeft} class="w-4 h-4" /> Voltar a lista de exercícios
      </button>
    </div>
  {/if}
</div>
